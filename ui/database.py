...
from models import Criterion, News
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(
                    ...
                    INSERT INTO criteria (
                        id, criterion_text, criteria_version, is_active,
            ...
            with self.get_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        """
                        INSERT INTO news (
                            id, title, url, content, source, search_query,
...
class ClickHouseManager:
    ...
    def get_events_by_source(self, source_hash: str, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/",
                params={"query": query, "database": self.database, "format": "JSONEachRow"},
            )
    ...
    def get_recent_events(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/", params={"query": query, "database": self.database}
            )
            response.raise_for_status()
            ...
            for line in lines:
                if line.strip():
                    ...
                    if len(values) >= len(headers):
                        ...
                        if (
                            stat.get("avg_confidence") is not None
                            and stat["avg_confidence"] != "\\N"
                        ):
                            stat["avg_confidence"] = float(stat["avg_confidence"])
                        if (
                            stat.get("avg_latency_ms") is not None
                            and stat["avg_latency_ms"] != "\\N"
                        ):
