...
def main():
    ...
    with st.sidebar:
        st.header("📊 Навигация")
        page = st.selectbox(
            "Выберите раздел:", ["📝 Анализ текста", "📈 Статистика", "🔍 История", "⚙️ Настройки"]
        )
...
def show_text_analysis_page():
    ...
    with st.form("text_analysis_form"):
        ...
        with col1:
            st.text_input("URL источника (опционально):", placeholder="https://example.com/article")

        with col2:
            st.date_input("Дата источника (опционально):", value=None)

        st.checkbox("Принудительная перепроверка")
...
def show_statistics_page():
    ...
    with col1:
        st.selectbox("Период:", [7, 30, 90], index=1)
...
def show_history_page():
    ...
    if events:
        ...
        if show_matches_only:
            events_df = events_df[events_df["is_match"]]
