...
def main():
    ...
    results = {
        ...
        "load_tests": {},
        "security_tests": {},
        "overall": {"total_tests": 0, "passed_tests": 0, "failed_tests": 0, "total_time": 0},
    }
    ...
    for test_file, description in unit_test_files:
        success, duration = run_command(f"python3 -m pytest {test_file} -v --tb=short", description)
    ...
    success, duration = run_command(
        "python3 -m pytest tests/test_load.py -v --tb=short", "Load тесты производительности"
    )
    ...
    success, duration = run_command(
        "python3 -m pytest tests/test_security.py -v --tb=short", "Security тесты безопасности"
    )
    ...
    success, duration = run_command("python3 integration_test.py", "Интеграционный тест системы")
    ...
    success_rate = (
        (results["overall"]["passed_tests"] / results["overall"]["total_tests"] * 100)
        if results["overall"]["total_tests"] > 0
        else 0
    )
...
"""
    ...
    for test_name, test_result in results["unit_tests"].items():
        status = "✅ ПРОЙДЕН" if test_result["success"] else "❌ ПРОВАЛЕН"
        report_content += (
            f"- **{test_result['description']}:** {status} ({test_result['duration']:.2f}s)\n"
        )
