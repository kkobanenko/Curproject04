...
def analyze_text_task(
    ...
    try:
        ...
        if existing_source and not force_recheck:
            ...
            return {"status": "skipped", "source_hash": source_hash, "reason": "already_processed"}
        ...
        if not criteria:
            ...
            return {"status": "error", "source_hash": source_hash, "reason": "no_active_criteria"}
        ...
        if not llm_client.health_check():
            ...
            return {"status": "error", "source_hash": source_hash, "reason": "llm_unavailable"}
        ...
        for i, criterion in enumerate(criteria):
            ...
            # Создаем событие
            logger.info(
                f"source_date type: {type(source.source_date)}, value: {source.source_date}"
            )
            event = Event(
                source_hash=source_hash,
                source_url=source_url,
            ...
            logger.info(
                f"event.source_date type: {type(event.source_date)}, value: {event.source_date}"
            )
