...
class TestFullFlow:
    ...
    def test_text_analysis_flow(self, mock_redis, mock_postgres, mock_clickhouse, mock_ollama, sample_text_data):
    ...
    def test_concurrent_processing(self, mock_redis, mock_postgres, mock_clickhouse, mock_ollama, sample_text_data):
    ...
    def test_data_consistency(self, mock_redis, mock_postgres, mock_clickhouse, mock_ollama, sample_text_data):
        """Тест консистентности данных"""
        with patch('sys.path', sys.path + [os.path.join(os.path.dirname(__file__), '..', 'worker')]):
            from tasks import analyze_text_task, normalize_text, compute_hash
            ...
            result = analyze_text_task(sample_text_data["text"], sample_text_data["source_url"], sample_text_data["source_date"])
            assert result['source_hash'] == text_hash

    def test_performance_metrics(self, mock_redis, mock_postgres, mock_clickhouse, mock_ollama, sample_text_data):
