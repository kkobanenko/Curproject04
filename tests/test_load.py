...
class TestConcurrentProcessing:
    ...
    def test_concurrent_text_analysis(
        self, mock_redis, mock_postgres_manager, mock_clickhouse_manager, mock_ollama
    ):
...
class TestMemoryUsage:
    ...
    def test_memory_usage_under_load(
        ...
        with patch(
            ...
            for i in range(50):
                # Увеличиваем размер
                text = f"Длинный текст для анализа {i} " * 100
...
class TestWorkerScalability:
    ...
    def test_worker_scalability(
        ...
        with patch(
            ...
            # В тестовой среде с моками это может не работать,
            # поэтому просто проверяем что тест завершается
